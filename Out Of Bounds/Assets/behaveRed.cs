...
public class behaveRed : MonoBehaviour {
	...
	void OnTriggerEnter2D(Collider2D rigid){
		...
		case 1 : 
			...
			rigid.gameObject.rigidbody2D.velocity = new Vector2(0f,-1*rigid.gameObject.rigidbody2D.velocity.x);
			rigid.gameObject.rigidbody2D.AddForce(new Vector2(0,160f));
			break;
		case 2 : 
			thePosition.x = sortie.position.x+0.5f ;
			thePosition.y = sortie.position.y-0.5f ;
			rigid.gameObject.rigidbody2D.velocity = new Vector2(rigid.gameObject.rigidbody2D.velocity.y,0f);
